...
* Go to [AWS Regions and Endpoints](https://docs.aws.amazon.com/general/latest/gr/rande.html) and copy the value of your region;
* Paste it to the `settings.py` file:
```python
...
...
```
### 2.2. Create [S3 bucket](https://s3.console.aws.amazon.com/s3/) service
S3 service is the place where we going to collect static files. \
#### Permissions block public access settings (bucket settings):
...
* Copy the name of the S3 bucket and paste it to `settings.py` file:
```python
# .../django_server/django_server/settings.py
...
...
...
```
### 2.3. Create RDS database service
...
- Copy the `password` to the `settings.py` file and provide your `GMail email`:
```python
# .../django_server/django_server/settings.py
...
