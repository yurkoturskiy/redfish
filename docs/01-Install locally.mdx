...
* Execute the `tmux_setup.py` script with a `python` version `>=3.6`
```console
$ python tmux_setup.py
```
* Some keys combination to play with:
   * `ctrl+b` `w` - navigate through windows;
   * `ctrl+b` `d` - close tmux, but save session (minimize);
   * `ctrl+d` - terminate tmux and session;
   * `$ tmux a` - go back to tmux after minimizing it;
